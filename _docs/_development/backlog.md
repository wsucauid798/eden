...
- [ ] Physics integration against **Jolt** (via `JoltPhysicsSharp`) — matches Godot 4's default 3D physics backend so viewer and server share the same engine. Bullet was the inherited-OpenSim default and was dropped once the fresh-start decision landed.
