...
- [x] ~~Triage whether existing NUnit tests pass; mark broken ones for later~~ obsoleted — the NUnit suite died with the legacy tree; new-tree suite is xUnit, 60 tests across 6 per-source-project assemblies under `tests/Eden.*.Tests/`.
- [x] ~~Decide what to do with root `eden.sln` stub~~ `eden.sln` is now the live solution for the new tree (8 projects in sln + `Eden.Viewer` excluded as Godot-world).
...
- [x] ~~Pin monorepo layout~~ `src/` + `tests/` split. Product under `src/` (`Eden.Shared`, `Eden.Server`, `Eden.Client`, `Eden.Launcher`, `Eden.Logging`, `Eden.Scripting` with nested `Host/`, `Eden.Viewer`); one test project per source project under `tests/Eden.<X>.Tests/`. `Eden.Viewer.Tests` is scaffolded but kept out of `eden.sln` (Godot-world, mirrors Eden.Viewer). Contributor docs at `_docs/_development/` (BUILDING / TESTING / backlog); private design at `_docs/_design/`.
...
- [ ] Scene graph running against new transport — partial: server has a prim registry (`EdenServer._prims`), `SpawnPrimAsync(pose, scale, flags, behavior?)` creates prims (static or `PrimFlags.Physical` dynamic), `ClientTouchPrim` wire frame dispatches `[OnTouch]` onto the prim's behavior via `ServerSelfContext`. `ServerSelfContext` mutations (`MoveTo`, `RotateBy`, `SetScale`) broadcast `PrimUpdate` to every session; new joiners receive existing prims on `ClientHello`. World-wide state (name, time of day, wind, weather, gravity) modelled as `WorldState` with a `WorldConfig` at server construction; a 1 Hz world-clock ticker advances `TimeOfDayHours` by `24 / DayLengthSeconds` per second and broadcasts `WorldStateUpdate` to every session. Scripts read via `IWorldContext.World`; viewers mirror via `ViewerClient.RemoteWorldState`. Full scene graph (children, parenting, visibility, culling) still TODO.
- [x] ~~C# scripting API surface~~ `Eden.Scripting` — `EdenBehavior` + event/capability attributes + context interfaces. Samples in `tests/Eden.Scripting.Tests/ScriptingSamplesCompileTest.cs` are a compile-check.
- [x] ~~Roslyn-based script host; trust-model sandbox initially~~ `Eden.Scripting.Host.BehaviorHost` — pre-compiled assembly loading (Roslyn-from-source deferred), reflection-based handler dispatch, lifecycle, error isolation. Per-binding `[OnTimer(s)]` / `[OnTick(s)]` scheduler via `PeriodicTimer`. Attribute-instance filter so `[OnChat(Channel=…)]` fires only on matching channels. Server-side dispatch wired: `ClientTouchPrim` wire frame → `[OnTouch]`; `ChatMessage` wire frame → `[OnChat(Channel=…)]` with `From` normalised server-side (spoof-guard). 17 scripting-host tests + 15 server-tests exercising the real dispatch pipeline. Trust model only; sandboxing deferred to Phase 6.
...
- [x] ~~Integration tests that spin up a server and hit endpoints~~ 60 tests across 6 per-project test assemblies: handshake, multi-client avatar registry, broadcast + spoof-guard, QUIC end-to-end, ViewerClient mirror (incl. `RemotePrims`), chat broadcast + dispatch, touch dispatch, gravity via Jolt.
- [x] ~~Lock the scripting API shape with a worked sample in `_docs/_design/`~~ Design doc at `_docs/_design/scripting-model.md`; scaffolded `Eden.Scripting/` project with `EdenBehavior` + event/capability attributes + `ISelfContext`/`IWorldContext`. Sample behaviors (`DoorScript`, `VendingMachine`, `SerialDemo`) in `tests/Eden.Scripting.Tests/ScriptingSamplesCompileTest.cs` are a compile-check of the API — if the shape drifts, the build breaks.
