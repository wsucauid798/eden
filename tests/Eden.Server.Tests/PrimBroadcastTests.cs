...
public class PrimBroadcastTests
{
    ...
    public async Task Late_Joiner_Receives_Existing_Prims()
    {
        ...
        // PrimUpdate frames for existing prims are sent after ServerHello,
        // so ConnectAsync returns before the receive loop has consumed them.
        var seen = await WaitForAsync(
            () => late.RemotePrims.ContainsKey(primId),
            TimeSpan.FromSeconds(1));
        Assert.True(seen, "Late-joining viewer did not receive existing prim on hello.");
    }
